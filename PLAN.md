...
> **Note (migration):** this is the original design, written when MiniWind was a
> Fio *plugin*. MiniWind is now the **integrated game layer** of this branch
> (`game/`), installed natively rather than discovered as a plugin. Where this
> document says "plugin hook", "`plugin.py`" or "`plugins.miniwind`", read the
> native equivalent: `game/host.py` (`MiniwindGame`) installed via
> `game.install()`. See [`ARCHITECTURE.md`](ARCHITECTURE.md) for the migration
> outcome. The system design below (entity model, low-frequency AI, schedules,
> save/load, editor extension points) is unchanged and still current.
