...
* **MiniWind is the built-in game** — it is installed natively at startup (not a
  ...
  world streaming, residency and simulation tiers come from Fio's core Big World
  plugin (a map opts in with its `BigWorldSettings` entity) — see
  ARCHITECTURE.md §7.
* **No guns anywhere** — the gun/cigarette HUD art is deleted and legacy monster
  attack sounds point at the fantasy melee/bow sounds. The engine's shooting is
  kept and given fantasy meaning: the fire buttons are handed to MiniWind, which
  swings, looses arrows or casts spell bolts. All combat is melee, archery or
  magic.
* **No demo content** — Fio's example maps and the **procedural map generator**
  (menu item, widget and CLI tools) are removed. The shipped map is
...
2. **Shooting** (`engine/logic_thread.py`, `engine/threaded_game_state.py`) — left
   and right mouse queue primary and secondary shots; `_handle_shooting` hands
   them to the installed `player_fire_handler` (MiniWind: attack / cast), and the
   resulting arrows and spell bolts use the projectile pipeline.
3. **RPG damage mitigation** (`engine/logic_thread.py`) — an optional
   `_player_damage_filter` turns raw incoming damage into a post-armour/-resistance
   amount before health is reduced.
4. **World pause** (`engine/logic_thread.py`, `engine/monster_ai.py`) — a
   `gameplay_paused` flag freezes the world while a menu is open without stopping
   the plugin tick that drives the menu.
5. **HUD suppression** (`engine/qt_game_view.py`) — the game suppresses the stock
   health/weapon HUD (`_suppress_default_hud`) and draws its own via `render.overlay`.
6. New fantasy assets: `assets/sounds/{melee,bow}.wav`, `assets/sprites/monsters/arrow.png`.
